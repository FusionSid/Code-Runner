...
LANGUAGES = Literal["python", "rickroll-lang"]
...
async def cleanup(random_code):
    ...
    os.system(f"docker container kill {container} >/dev/null 2>&1") # >/dev/null 2>&1 just makes it try to not print the output to terminal
